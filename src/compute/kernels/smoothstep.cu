...
__global__ void smoothstep(ConstrainedGrid heights, Vertex offset, BrushDab brush_dab) {
  ...
  float y_diff = (float)row - brush_dab.circle.center.y;
  float distance = sqrtf(x_diff * x_diff + y_diff * y_diff);
  ...
  float factor = 1 - distance / brush_dab.circle.radius;
  float height = heights[row][col] + factor * factor * (3 - 2 * factor) * brush_dab.intensity;
  heights[row][col] = max(heights.range.min, min(heights.range.max, height)); // clamp
}
