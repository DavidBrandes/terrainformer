...
__global__ void marching_squares_part_1_privatized(compute::CGrid heights, int* __restrict__ count, int max_count,
  ...
  __shared__ int2 tmp_coordinates_s[capacity];
  __shared__ float tmp_thresholds_s[capacity];
  __shared__ int block_count_s;
  __shared__ int global_count_s;
...
#pragma unroll 16
    for (int i = 0; i < compute_layers; ++i) {
      ...
      int type = compute_type(top_left, top_right, bottom_right, bottom_left, threshold);
      int thread_count = count_for_type(type);
      ...
      int block_count = block_count_ref.fetch_add(thread_count, cuda::memory_order_relaxed);
  ...
  if (thread_id == 0 && block_count_s > 0) {
    ...
    global_count_s = global_count_ref.fetch_add(block_count_s, cuda::memory_order_relaxed);
  }

  __syncthreads();

  for (int i = thread_id; i < block_count_s; i += stride) {
    tmp_coordinates[global_count_s + i] = tmp_coordinates_s[i];
    tmp_thresholds[global_count_s + i] = tmp_thresholds_s[i];
  }
}
